...
static long bio_count_reads_callback(BIO *bio, int cmd, const char *argp, size_t len,
...
# endif
    ...
    BIO_set_callback_ex(SSL_get_rbio(con), bio_count_reads_callback);
    if (s_msg) {
#ifndef OPENSSL_NO_SSL_TRACE
        if (s_msg == 2)
    ...
    for (;;) {
        ...
        if (read_from_sslcon) {
            ...
            if ((!async || !SSL_waiting_for_async(con))
                ...
                /*
                 * If initialization fails and there are no reads, then there
                 * must be a fatal error in configuration.
                 */
